...
public class CharacterParser {
    private final static Log log = LogFactory.getLog(CharacterParser.class);
    ...
    private int getChsAscii(String chs) {
        ...
        try {
            ...
            if (bytes == null || bytes.length > 2 || bytes.length <= 0) {
                log.error("Error:getChsAscii->illegal resource string");
                throw new RuntimeException("Error:CharacterParser.getChsAscii(String chs)->illegal resource string");
            }
            if (bytes.length == 1) {
                asc = bytes[0];
            }
            ...
            log.error("Error:getChsAscii", e);
            throw new RuntimeException("Error:CharacterParser.getChsAscii(String chs)");
        }
        return asc;
    }
    ...
    public String getSelling(String chs) {
        return getSellingCommon(chs, false, null);
    }

    public String getSelling(String chs, String unkownTag) {
        return getSellingCommon(chs, false, unkownTag);
    }

    public String getSellingWithSpace(String chs) {
        return getSellingCommon(chs, true, null);
    }

    public String getSellingWithSpace(String chs, String unkownTag) {
        return getSellingCommon(chs, true, unkownTag);
    }

    private String getSellingCommon(String chs, boolean isPySpace, String unkownTag) {
        String key, value;
        buffer = new StringBuilder();
        boolean isEng = false;
        boolean isChs = false;
        for (int i = 0; i < chs.length(); i++) {
            boolean tmpIsEng = false;
            boolean tmpIsChs = false;
            key = chs.substring(i, i + 1);
            if (key.getBytes().length >= 2) {
                tmpIsChs = true;
                value = convert(key);
                if (value == null) {
                    if (null == unkownTag || unkownTag.length() <= 0) {
                        value = key;
                    } else {
                        value = unkownTag;
                    }
            ...
            if (isPySpace) {
                if (isEng && tmpIsChs) {
                    buffer.append(" ");
                } else if (isChs && tmpIsChs) {
                    buffer.append(" ");
                } else if (isChs && tmpIsEng) {
                    buffer.append(" ");
                }
            }
            buffer.append(value);
