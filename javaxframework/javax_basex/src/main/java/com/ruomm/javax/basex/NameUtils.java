...
//			"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789/_";
    ...
    public static String generateName(boolean isRandom) {
        ...
        return generateNameWithFamilyName(null, RATE_TWO, RATE_THREE);
    }
    ...
    public static String generateNameWithFamilyName(String familyName, boolean isRandom) {
        if (!isRandom) {
            return NAME_DEFAULT;
        }
        return generateNameWithFamilyName(familyName, RATE_TWO, RATE_THREE);
    }

    public static String generateNameWithFamilyName(String familyName) {
        return generateNameWithFamilyName(familyName, RATE_TWO, RATE_THREE);
    }

    public static String generateNameBySize(int nameSize) {
        return generateNameBySizeWithFamilyName(null, nameSize);
    }

    public static String generateNameBySizeWithFamilyName(String familyName, int nameSize) {

        Random random = new Random();
        List<String> listNameFirst = new ArrayList<String>();
        if (null == familyName || familyName.length() <= 0) {
    ...
    public static String generateName(int rateTwo, int rateThree) {
        return generateNameWithFamilyName(null, rateTwo, rateThree);
    }

    public static String generateNameWithFamilyName(String familyName, int rateTwo, int rateThree) {
        int r2 = rateTwo;
        int r3 = rateThree;
        if (r2 < 0 || r2 > 100) {
            r2 = RATE_TWO;
        ...
        if (iRandom < r2) {
            return generateNameBySizeWithFamilyName(familyName, 2);
        } else if (iRandom < r2 + r3) {
            return generateNameBySizeWithFamilyName(familyName, 3);
        } else {
            return generateNameBySizeWithFamilyName(familyName, 4);
        }
    }
