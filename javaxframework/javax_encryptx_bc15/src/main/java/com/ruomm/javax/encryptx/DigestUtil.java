...
public class DigestUtil {
    private final static Log log = LogFactory.getLog(DigestUtil.class);
    ...
    private final static String METHOD_SHA3_256 = "SHA3-256";
    private final static String DIGEST_MODE_JDK = "JDK_MODE";
    private final static String DIGEST_MODE_BC = "BC_MODE";
    //	private final static String DIGEST_MODE_CC = "CC_MODE";
    private static String DIGEST_DEFAULT;
    private final static boolean RESULT_UPPER_CASE = false;
    public final static int BUFFER_SIZE = 1024;

    static {
        boolean isBC = false;
        try {
            Class<?> clsProvider = Class.forName("org.bouncycastle.jce.provider.BouncyCastleProvider");
            Class<?> clsMd5 = Class.forName("org.bouncycastle.crypto.digests.MD5Digest");
            if (null != clsProvider && null != clsMd5) {
                isBC = true;
            }
        } catch (Exception e) {
            // TODO: handle exception
            log.debug("Error:DigestUtil.static->BouncyCastleProvider md5 not found!", e);
            isBC = false;
        }
        if (isBC) {
            DIGEST_DEFAULT = DIGEST_MODE_BC;
        } else {
            DIGEST_DEFAULT = DIGEST_MODE_JDK;
        }
    }

    public static void setDigestModeJDK() {
        DIGEST_DEFAULT = DIGEST_MODE_JDK;
    }

    public static void setDigestModeBC() {
        DIGEST_DEFAULT = DIGEST_MODE_BC;
    }

//	public static void setDigestModeCC() {
//		DIGEST_DEFAULT = DIGEST_MODE_CC;
//	}
    ...
    public static String encodingSHA3_256(String data) {
        return DigestCCMode.encodingByMessageDigest(data, METHOD_SHA3_256, RESULT_UPPER_CASE);
    }
    ...
    public static String encodingSHA3_256(String data, String charsetName) {
        return DigestCCMode.encodingByMessageDigest(data, METHOD_SHA3_256, charsetName, RESULT_UPPER_CASE);
    }
    ...
    public static String encodingInputStreamSHA3_256(InputStream sourceInputStream) {
        return DigestCCMode.getMessageDigestForInputStreamByCCMode(sourceInputStream, METHOD_SHA3_256, RESULT_UPPER_CASE);
    }
    ...
    public static String getMessageDigest(byte[] sourceData, String digestMethod) {
        if (DIGEST_DEFAULT.equals(DIGEST_MODE_JDK)) {
            return getMessageDigestByJDKMode(sourceData, digestMethod);
        } else if (DIGEST_DEFAULT.equals(DIGEST_MODE_BC)) {
            return DigestBCMode.getMessageDigestByBCMode(sourceData, digestMethod, RESULT_UPPER_CASE);
        } else {
            return getMessageDigestByJDKMode(sourceData, digestMethod);
        }
    }
...
//	}
    ...
    public static String getMessageDigestForInputStream(InputStream inputStream, String saltStr, String charsetName,
        ...
        if (DIGEST_DEFAULT.equals(DIGEST_MODE_JDK)) {
            ...
            return DigestBCMode.getMessageDigestForInputStreamByBCMode(inputStream, saltStr, charsetName, digestMethod, RESULT_UPPER_CASE);
        } else {
            return getMessageDigestForInputStreamByJDKMode(inputStream, saltStr, charsetName, digestMethod);
        }
    }
...
//	public static String getMessageDigestForInputStreamByCCMode(InputStream sourceInputStream, String digestMethod) {
//		String resultData;
//		try {
//			if (isMethodEqual(METHOD_MD2, digestMethod)) {
