...
public class SM2EncDecUtils {
    ...
    public static SM2KeyVO generateKeyPair() {
        ...
        if (SMConfig.isLog) {
            log.debug("generateKeyPair->公钥(Hex)：" + Util.byteToHex(publicKey.getEncoded(false)));
            log.debug("generateKeyPair->私钥(Hex)：" + Util.byteToHex(privateKey.toByteArray()));
            log.debug("generateKeyPair->公钥(base64)：" + Base64Utils.encodeToString(publicKey.getEncoded(false)));
    ...
    public static byte[] encrypt(byte[] publicKey, byte[] data, boolean isC1C3C2, boolean isHard) {
        ...
        byte[] c3 = new byte[32];
        cipher.Dofinal(c3);
        byte[] c1Result = c1.getEncoded(false);
        if (isHard) {
