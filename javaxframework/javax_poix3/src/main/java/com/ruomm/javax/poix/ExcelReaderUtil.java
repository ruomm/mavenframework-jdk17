...
public class ExcelReaderUtil {
    ...
    public static <T> List<T> readExcel(InputStream inputStream, String sheetType, Class<T> tCls) {
        if (null == inputStream) {
            new JavaxCorexException(JavaxCorexException.CODE_ERR_JAVAX_COREX, "ExcelReaderUtil.readExcel->" + "读取错误->inputStream为空，无法获取Excel读取的输入流！");
        }
        ...
        try {
                ...
                if (null == sheet) {
                    if (excelReaderSheet.getSheetIndex() >= 0) {
                        ...
                        if (excelReaderSheet.getSheetIndex() >= numberOfSheets) {
                            ...
                            new JavaxCorexException(JavaxCorexException.CODE_ERR_JAVAX_COREX, "ExcelReaderUtil.readExcel->" + "读取设置错误->sheet数量为"
                        ...
                        if (numberOfSheets != 1) {
                            ...
                            new JavaxCorexException(JavaxCorexException.CODE_ERR_JAVAX_COREX, "ExcelReaderUtil.readExcel->" + "读取设置错误->excle的sheet数量为"
            ...
            if (null == sheet) {
                ...
                new JavaxCorexException(JavaxCorexException.CODE_ERR_JAVAX_COREX, "readExcel->" + "读取设置错误->无法依据DefExcelReaderSheet注解获取到excel表单，无法读取！");
            }
            ...
            if (firstRowIndex > lastRowIndex) {
                ...
                new JavaxCorexException(JavaxCorexException.CODE_ERR_JAVAX_COREX, "ExcelReaderUtil.readExcel->" + "读取设置错误->excel读取的开始行号大于结束行号数");
            }
            ...
            for (int rIndex = firstRowIndex; rIndex <= lastRowIndex; rIndex++) {
                ...
                if (isThrowErr) {
                    list.clear();
                    new JavaxCorexException(JavaxCorexException.CODE_ERR_JAVAX_COREX, "CellWriteResult->解析" + rIndex + "行" + errorIndex + "列"
            ...
            new JavaxCorexException(JavaxCorexException.CODE_ERR_JAVAX_COREX, "ExcelReaderUtil.readExcel->读取Excel文件时候发生异常");
...
//		return max;
    ...
    private static <T> CellWriteResult writeCellToObjcet(ExcelReaderCell excelCell, Cell cell, T t, int rowIndex) {
        ...
        try {
            ...
            } else if (cellType == Cell.CELL_TYPE_STRING || cellType == Cell.CELL_TYPE_FORMULA) {
                String cellVal = null;
                ...
                if (cellFieldType == int.class || cellFieldType == Integer.class) {
                    if (StringUtils.isEmpty(cellVal)) {
                        if (cellFieldType == Integer.class) {
                            ...
                            new JavaxCorexException(JavaxCorexException.CODE_ERR_JAVAX_COREX, "CellWriteResult->解析为int类型时候值不能为空！");
                        }
                    } else {
                        BigDecimal bigDecimal = new BigDecimal(cellVal).setScale(0, excelCell.getNumRoundingMode());
                        invokeSetCellValue(excelCell, t, bigDecimal.intValue());
                    ...
                    if (StringUtils.isEmpty(cellVal)) {
                        if (cellFieldType == Long.class) {
                            ...
                            new JavaxCorexException(JavaxCorexException.CODE_ERR_JAVAX_COREX, "CellWriteResult->解析为long类型时候值不能为空！");
                        }
                    } else {
                        BigDecimal bigDecimal = new BigDecimal(cellVal).setScale(0, excelCell.getNumRoundingMode());
                        invokeSetCellValue(excelCell, t, bigDecimal.longValue());
                    ...
                    if (StringUtils.isEmpty(cellVal)) {
                        if (cellFieldType == Short.class) {
                            ...
                            new JavaxCorexException(JavaxCorexException.CODE_ERR_JAVAX_COREX, "CellWriteResult->解析为short类型时候值不能为空！");
                        }
                    } else {
                        BigDecimal bigDecimal = new BigDecimal(cellVal).setScale(0, excelCell.getNumRoundingMode());
                        invokeSetCellValue(excelCell, t, bigDecimal.shortValue());
                    ...
                    if (StringUtils.isEmpty(cellVal)) {
                        if (cellFieldType == Byte.class) {
                            ...
                            new JavaxCorexException(JavaxCorexException.CODE_ERR_JAVAX_COREX, "CellWriteResult->解析为byte类型时候值不能为空！");
                        }
                    } else {
                        BigDecimal bigDecimal = new BigDecimal(cellVal).setScale(0, excelCell.getNumRoundingMode());
                        invokeSetCellValue(excelCell, t, bigDecimal.byteValue());
                    ...
                    if (StringUtils.isEmpty(cellVal)) {
                        if (cellFieldType == Float.class) {
                            ...
                            new JavaxCorexException(JavaxCorexException.CODE_ERR_JAVAX_COREX, "CellWriteResult->解析为float类型时候值不能为空！");
                        }
                    } else {
                        BigDecimal bigDecimal = new BigDecimal(cellVal);
                        if (excelCell.getNumScale() >= 0) {
                    ...
                    if (StringUtils.isEmpty(cellVal)) {
                        if (cellFieldType == Double.class) {
                            ...
                            new JavaxCorexException(JavaxCorexException.CODE_ERR_JAVAX_COREX, "CellWriteResult->解析为double类型时候值不能为空！");
                        }
                    } else {
                        BigDecimal bigDecimal = new BigDecimal(cellVal);
                        if (excelCell.getNumScale() >= 0) {
                    ...
                    if (StringUtils.isEmpty(cellVal)) {
                        if (cellFieldType == Boolean.class) {
                            ...
                            new JavaxCorexException(JavaxCorexException.CODE_ERR_JAVAX_COREX, "CellWriteResult->解析为boolean类型时候值不能为空！");
                        }
                    } else {
        ...
        try {
            ...
            if (!StringUtils.isEmpty(sheetName)) {
                ...
                if (index >= numberOfSheets) {
                    ...
                    new JavaxCorexException(JavaxCorexException.CODE_ERR_JAVAX_COREX, "ExcelReaderUtil.readExcel->读取错误，excle的sheet数量为" + numberOfSheets
                            + "，声明的序号" + index + "超界");
                }
            ...
            if (firstRowIndex > lastRowIndex) {
                ...
                new JavaxCorexException(JavaxCorexException.CODE_ERR_JAVAX_COREX, "ExcelReaderUtil.readExcel->读取错误，读取excel文件开始行号大于结束行号数");
            }
            for (int rIndex = firstRowIndex; rIndex <= lastRowIndex; rIndex++) {
                Row row = sheet.getRow(rIndex);
                ...
                if (firstCellIndex > lastCellIndex) {
                    ...
                    new JavaxCorexException(JavaxCorexException.CODE_ERR_JAVAX_COREX, "ExcelReaderUtil.readExcel->读取错误，读取exlce单元行时候开始列号大于结束列号");
                }
